...
con = mysql.connector.connect(host="localhost",user="root",password="surya525",database="db")
...
def issuebook():
   ...
   '''con = mysql.connector.connect(host="localhost", user="root", password="surya525", database="db", port=3306)
   cur = con.cursor()'''
   ...
   for k in cur:
      ...
      if (a == input_bookid): # checks if the book id inputed is valid by cross checking with the existing book ids in the book database
         valid = True
   cur.execute("SELECT SRN FROM users WHERE SRN = %s", (input_userid,))
   user_exists = cur.fetchone()

   if not user_exists:
        messagebox.showwarning("ERROR", "Invalid SRN. User not found.")
        return
   cur.execute("select * from borrowers where SRN="+ str(input_userid))
